...
def pull_mint_addresses():
    ...
    min_associated_nfts_to_be_valid = 100
    ...
    with open("smb_mint_addresses.json", "w+") as fp:
...
def pull_mint_stats():
    ...
    prices = list()
    exchange_listings = defaultdict(int)
        ...
        def get_data(index: int):
            ...
            try:
                ...
                try:
                    ...
                    exchange_listings[listing['exchange_readable']] += 1
    ...
    if len(exchange_listings) > 0:
        for exchange_listing_name, count in exchange_listings.items():
            print(f"{count} Money Boys are listed on {exchange_listing_name}")
